 ...
 * - FatNum is a class that represents a very large integer and implements basic
 * arithmetic operations.
 * - The class should be able to handle integers of any size by using a
 * datastructure to store hexadicimal chunks stored as strings of the number.
 * - The class should be able to handle negative numbers and provides the
 * following fiels :
 ...
 * - A chunksize integer that defines the size of the chunks associated with a
 * default value of 8
 */
...
public:
    // Constructors
    FatNum()
        : sign(1), chunkSize(DEFAULT_CHUNK_SIZE),
          length(0) {}; // Default constructor
    // TODO: Implement the following methods : fromInt, fromString to be used in
    // the constructors
    FatNum(const string& num); // Constructor with string argument
    FatNum(const int num); // Constructor with int argument -> better approach
                           // for relatively small numbers -> use limits.h to
                           // get INT_MAX and INT_MIN
    FatNum(
        const string& num,
        const int chunkSize); // Constructor with string and chunkSize arguments
    FatNum(const int num,
           const int chunkSize); // Constructor with int and chunkSize arguments
    ...
    int toInt(FatNum n) const; // Returns the integer value of the FatNum if
                               // FatNum is small enough to fit in an int
...
// TODO: check how to improve for very very large numbers because memory will be
// a problem
std::vector<std::string> toHexChunks(const std::string& decimalStr,
                                     int chunkSize)
{
